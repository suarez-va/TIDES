...
def print_mo_occ(rt_mf):
    mo_occ = rt_mf.mo_occ
    rt_mf.log.note(f"Molecular Orbital Occupations: {' '.join(map(str,mo_occ))} \n")
...
def print_dipole(rt_mf):
    dipole = rt_mf.dipole
    rt_mf.log.note(f"Total Dipole Moment [X, Y, Z] (AU): {' '.join(map(str,dipole[0]))} \n")
    if len(dipole) > 1:
        for index, fragment in enumerate(dipole[1:]):
            rt_mf.log.note(f"Fragment {index + 1} Dipole Moment [X, Y, Z] (AU): {' '.join(map(str,fragment))} \n")
